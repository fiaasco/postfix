...
testinfra_hosts = testinfra.utils.ansible_runner.AnsibleRunner(
    os.environ['MOLECULE_INVENTORY_FILE']).get_hosts('postfix-debian-11')
...
def test_postfix_tls_main(host):
    ...
    assert f.contains('smtpd_tls_cert_file = /etc/ssl/postfix-debian-11/postfix_postfix-debian-11.chain')
    assert f.contains('smtpd_tls_key_file = /etc/ssl/postfix-debian-11/postfix_postfix-debian-11.key')
...
def test_postfix_tls_chain(host, item):
    """ basic configuration checks for TLS
    """
    f = host.file('/etc/ssl/postfix-debian-11/postfix_postfix-debian-11.%s' % item)
    assert f.exists
